...
public class BirdsFlock : MonoBehaviour {
	...
	private BirdsFlock[] otherFlocks;       //Unity Flocks in the group
	...
	void Start ()
	{
		...
		if (transform.parent)
		{
			tempFlocks = transform.parent.GetComponentsInChildren<BirdsFlock>();
		}
		
		//Assign and store all the flock objects in this group
		objects = new Transform[tempFlocks.Length];
		otherFlocks = new BirdsFlock[tempFlocks.Length];
		
		for(int i = 0;i<tempFlocks.Length;i++)
		{
			objects[i] = tempFlocks[i].transform;
			otherFlocks[i] = (BirdsFlock)tempFlocks[i];
		}
		
		//Null Parent as the flock leader will be BirdsFlockController object
		...
		for(int i = 0;i<objects.Length;i++)
		{
			...
			if (transform != transformComponent)
			{
				...
				if (d < followRadius)
				{
					...
					f = d / followRadius;
					BirdsFlock tempOtherFlock = otherFlocks[i];
